...
const Blogs = () => {
  ...
  // Fetch blogs when the component mounts or pagination parameters change
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const { page, limit } = pagination;
        const response = await axios.get('https://titoscornerwebservice.onrender.com/api/blogs/', {
          params: { page, limit }
        });
        ...
        if (Array.isArray(fetchedBlogs)) {
          ...
          console.error('Expected an array but got:', fetchedBlogs);
        }
      } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-8 pt-32">
      {blogs.map(({ id, headline, description, showFull, author }) => (
        <div
          key={id}
        ...
        >
          <article>
            ...
            <p
              className={`mt-3 text-sm leading-6 text-gray-600 text-justify ${showFull ? '' : 'line-clamp-3'}`}
            >
              {description}
            </p>
            <button
      ...
      <div className="flex justify-center mt-6">
        <button
          ...
          className="px-4 py-2 bg-gray-300 rounded-md text-sm font-semibold cursor-pointer"
        >
          Previous
        </button>
